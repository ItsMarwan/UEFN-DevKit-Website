...
export async function GET(req: NextRequest) {
  try {
    ...
    const { status, data } = await proxyFlaskQuota(req, auth.guildId);

    return NextResponse.json(data, { status, headers: { "Cache-Control": "no-cache" } });
  } catch (error) {
    console.error("Quota endpoint error:", error);
    return NextResponse.json(
      { status: "error", message: "Failed to get quota", error: error instanceof Error ? error.message : "Unknown error", timestamp: new Date().toISOString() },
      { status: 500 }
    );
  }
}
