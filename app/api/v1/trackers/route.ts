/**
 * Enterprise API - Trackers Endpoint
 * GET /api/v1/trackers
 * 
 * Fetch trackers with optional type filter
 */
...
export async function GET(req: NextRequest) {
  try {
    ...
    const response = await fetch(`${FLASK_API_URL}/api/v1/fetch`, {
      ...
      body: JSON.stringify({
        endpoint: "trackers",
        parameters: { limit: parseInt(limit), offset: parseInt(offset), type },
      }),
      cache: "no-store",
    });
    ...
    return NextResponse.json({ status: "error", message: "Failed to fetch trackers", error: error instanceof Error ? error.message : "Unknown error", timestamp: new Date().toISOString() }, { status: 500 });
  }
}
